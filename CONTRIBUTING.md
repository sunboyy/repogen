...
- Releasing:
    1. Update the unreleased version specified in `CHANGELOG.md`
    2. Remove `-next` from version variable in `main.go`
    3. Create a git tag `vX.X.X` without push to GitHub
    4. Run `goreleaser release` with GitHub token provided as an environment variable `GITHUB_TOKEN`
- Post-release:
    1. Bump version with `-next` suffix in version variable in `main.go`
    2. Add new unreleased version to `CHANGELOG.md`
